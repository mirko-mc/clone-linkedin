...
import { getOneProfile, getProfileUrl, patchAvatar } from "../fetchUrls";
...
function ProfilePage({ id }) {
  ...
  const getProfileData = async () => {
    const data = await getOneProfile(id);
  ...
  const handleFile = (ev) => {
    setFd((prev) => {
      ...
      prev.delete("avatar"); //ricordatevi di svuotare il FormData prima :)
      prev.append("avatar", ev.target.files[0]); //L'API richiede un "nome" diverso per ogni rotta, per caricare un'immagine ad un post, nel form data andra' inserito un valore con nome "post"
      return prev;
    });
  };
